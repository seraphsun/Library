...
public class HeaderFirst extends View {
    ...
    public HeaderFirst(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public HeaderFirst(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public HeaderFirst(Context context) {
        super(context);
        init(context);
    }
    ...
    private void init(Context context) {
        // 这个就是那个椭圆形图片
        initialBitmap = Bitmap.createBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.img_first_refresh));
        // 这个是第二个状态娃娃的图片，之所以要这张图片，是因为第二个状态和第三个状态的图片的大小是一致的，而第一阶段椭圆形图片的大小与第二阶段和第三阶段不一致，
        // 因此我们需要根据这张图片来决定第一张图片的宽高，来保证第一阶段和第二、三阶段的View的宽高一致
