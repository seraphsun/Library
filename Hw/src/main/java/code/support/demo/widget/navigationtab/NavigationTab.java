...
public class NavigationTab extends View implements ViewPager.OnPageChangeListener {
    ...
    public NavigationTab(final Context context) {
        this(context, null);
    }

    public NavigationTab(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NavigationTab(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NavigationTab);
        try {
            setStripColor(typedArray.getColor(R.styleable.NavigationTab_nt_color, DEFAULT_STRIP_COLOR));
            setTitleSize(typedArray.getDimension(R.styleable.NavigationTab_nt_size, DEFAULT_TITLE_SIZE));
            setStripWeight(typedArray.getDimension(R.styleable.NavigationTab_nt_weight, DEFAULT_STRIP_WEIGHT));
            setStripFactor(typedArray.getFloat(R.styleable.NavigationTab_nt_factor, DEFAULT_STRIP_FACTOR));
            setStripType(typedArray.getInt(R.styleable.NavigationTab_nt_type, StripType.LINE_INDEX));
            setStripGravity(typedArray.getInt(R.styleable.NavigationTab_nt_gravity, StripGravity.BOTTOM_INDEX));
            setTypeface(typedArray.getString(R.styleable.NavigationTab_nt_typeface));
            setInactiveColor(typedArray.getColor(R.styleable.NavigationTab_nt_inactive_color, DEFAULT_INACTIVE_COLOR));
            setActiveColor(typedArray.getColor(R.styleable.NavigationTab_nt_active_color, DEFAULT_ACTIVE_COLOR));
            setAnimationDuration(typedArray.getInteger(R.styleable.NavigationTab_nt_animation_duration, DEFAULT_ANIMATION_DURATION));
            setCornersRadius(typedArray.getDimension(R.styleable.NavigationTab_nt_corners_radius, DEFAULT_CORNER_RADIUS));

            // Get titles
            String[] titles = null;
            try {
                final int titlesResId = typedArray.getResourceId(R.styleable.NavigationTab_nt_titles, 0);
    ...
    /**
     * Resize interpolator to create smooth effect on strip according to inspiration design This is like improved accelerated and decelerated interpolator
     */
